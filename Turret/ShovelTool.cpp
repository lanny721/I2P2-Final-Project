...
void ShovelTool::Use() {
    ...
    if (getPlayScene()->mapState[y][x] == PlayScene::TILE_FLOOR) { // dig to dirt
        ...
        getPlayScene()->TileMapImages[y][x] = new Engine::Image("play/grass2.png", x * PlayScene::BlockSize, y * PlayScene::BlockSize, PlayScene::BlockSize, PlayScene::BlockSize);
        ...
        getPlayScene()->EarnMoney(getPlayScene()->Towers[y][x]->GetPrice());
        getPlayScene()->Towers[y][x] = nullptr;
    }
    // std::cout << getPlayScene()->mapState[y][x] << std::endl;
}
